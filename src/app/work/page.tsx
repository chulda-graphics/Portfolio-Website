...
export default function WorkPage() {
  return <PageMotion className="work-page">
    ...
    <section className="work-list">
      {projects.map((project, index) => <Link className={`work-row ${project.className} stack-card`} href={project.href} key={project.name}><div className="work-row-head"><span>{project.name}</span><span>{project.type}</span><b aria-hidden="true"><ArrowUpRight size={16} /></b></div><div className="work-row-art scale-reveal"><i /><i /><strong>{index === 0 ? "SEARCH LESS" : index === 1 ? "PLAY / 00:58" : "COMING SOON"}</strong></div><h2>{project.copy}</h2></Link>)}
    </section>
